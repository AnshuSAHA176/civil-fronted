...
import { getCitizenRTIs } from '../../features/rti.api'
...
export default function RTI() {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const response = await getCitizenRTIs()
    ...
    <section className="rti-page">
      <div className="rti-heading">
        <div>
          ...
          <h1>RTI requests</h1>
          <p>View the information requests associated with your CivicAI complaints.</p>
        </div>
        <Button type="button" onClick={() => navigate('/citizen/rti/new')}>
          <Plus size={17} aria-hidden="true" /> New RTI
        </Button>
      </div>
      ...
      {loading ? (
        <div className="page-loading"><Spinner /><span>Loading your RTI requests…</span></div>
      ) : error ? (
        <ErrorState title="Unable to load your RTI requests" description={error} onRetry={load} />
      ) : items.length === 0 ? (
        <EmptyState title="No RTI requests yet" description="You can file an RTI request about one of your complaints." action={<Button onClick={() => navigate('/citizen/rti/new')}>Create an RTI request</Button>} />
      ) : filtered.length === 0 ? (
        ...
        <div className="rti-list">
          {filtered.map((item) => (
            <article className="card rti-card" key={item.rti_id}>
              ...
              <Button type="button" variant="secondary" onClick={() => navigate(`/citizen/rti/${encodeURIComponent(item.rti_id)}`)}>View details</Button>
            </article>
          ))}
        </div>
      )}
